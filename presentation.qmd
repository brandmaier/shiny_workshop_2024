...
Add extra features to make the diagrams interactive with additional inputs (e.g., add a `selectInput` for subgroup selection of penguin species) or add a `rangeInput` to display only certain ranges of years, or make point size
adjustable by a given variable (`selectInput` or a `checkboxInput`).
...
This work is provided under [https://creativecommons.org/licenses/by/4.0/](Creative Commons Attribution 4.0 International License).

Some materials are based on work by Colin Rundel (https://github.com/rstudio-conf-2022/get-started-shiny/), which is provided under
